...
addr="C:\\Users\\user\\Dropbox\\shared with ubuntu\\disksim_github\\collected data(from disksim)\\AI input feature\\generate by current code\\"
data=np.loadtxt(addr+'info(wr_run1_ug3).txt',delimiter=' ',usecols=range(7))
...
while(finish!=len(data)):
    ...
    if duration_model.predict(tmp)[0][2]==np.max(duration_model.predict(tmp)):
        output=2   
    with open('duration(wr_run1_ug3).txt','a') as f:
