...
duration_model = tf.keras.models.load_model(addr+'duration_model(Financial2forssd).h5')
addr="C:\\Users\\user\\Dropbox\\shared with ubuntu\\disksim_github\\collected data(from disksim)\\AI input feature\\"
data=np.loadtxt(addr+'info(Financial2forssd).txt',delimiter=' ',usecols=range(7))
...
while(finish!=len(data)):
    ...
    if duration_model.predict(tmp)[0][2]==np.max(duration_model.predict(tmp)):
        output=2   
    with open('online(Financial2forssd)_duration.txt','a') as f:
