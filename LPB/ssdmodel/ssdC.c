...
int Y_add_Pg_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
  ...
  while(1)
  {
    if(Pg_node == NULL)
      break;
    if(Pg_node->logical_node_num == physical_node_num)//find
    {
...
void Y_add_Lg_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
  ...
  if(Pg_node != NULL) //have node
  {
    ...
    remove_mark_in_the_node(Pg_node,ptr_buffer_cache);
    /*if(Pg_node->page[phy_node_offset].exist != 0) // ¬O§_¦³ÄÝ©ó¦Û¤vªºLB¤w¦s¦bcache¤¤
    {
...
void show_result(buffer_cache *ptr_buffer_cache)
{
	...
	statistic_the_data_in_every_stage();
  printf("[Corn] LARGEcount = %d , Pg_hit_Lg=%d\n",LARGEcount, Pg_hit_Lg);
  ...
  printf(LIGHT_BLUE"[Corn] kick_channel_times = %d \n"NONE,kick_channel_times);
	printf(YELLOW"\n[Corn] kick_node=%d|kick_sum_page=%d|avg kick page=%lf|kick_LG_block_striping=%d\n"NONE, kick_node, kick_sum_page, (double)kick_sum_page/(double)kick_node, kick_LG_block_striping);
  printf(YELLOW"\n[Corn] kick_count=%d\n"NONE, kick_count);
