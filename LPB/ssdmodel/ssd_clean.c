...
static double ssd_move_page(int lpn, int from_blk, int plane_num, int elem_num, ssd_t *s)
{
    ...
    switch(s->params.copy_back) {
        ...
        case SSD_COPY_BACK_ENABLE:
            //ASSERT(metadata->plane_meta[plane_num].g_active_page == metadata->active_page);
            if (ssd_last_page_in_block(metadata->active_page, s)) 
						{
	    				if(metadata->block_usage[from_blk].rw_intensive == 1)
								_ssd_alloc_active_block(plane_num, elem_num, s , 1);
							else
                _ssd_alloc_active_block(plane_num, elem_num, s , 2);
            }
            break;
    ...
    if(metadata->block_usage[from_blk].rw_intensive == 1)
    cost += _ssd_write_page_osr(s, metadata, lpn, elem_num,1);
    else
    cost += _ssd_write_page_osr(s, metadata, lpn, elem_num,2);
...
int ssd_migrate_cold_data(int to_blk, double *mcost, int plane_num, int elem_num, ssd_t *s)
{
    ...
    for (i = 0; i < s->params.pages_per_block; i ++) {
        ...
        if (lpn != -1) {
            ...
            lba_table[lpn].ppn = to_blk * s->params.pages_per_block + i;
        }
...
{
    double cost = 0;
		char gc_data_file[100];
		FILE *pf_para = NULL;
		char tmp1[100],tmp2[100],tmp3[100],tmp4[100];//the char will been create the gc log file name
    plane_metadata *pm = &metadata->plane_meta[plane_num];
...
static double ssd_clean_block_fully(int plane_num, int elem_num, ssd_t *s)
{
    ...
    if(metadata->block_usage[blk].rw_intensive == 1)
			metadata->active_page = metadata->plane_meta[plane_num].r_active_page;
    else
			metadata->active_page = metadata->plane_meta[plane_num].g_active_page;
 ...
 */
void find_min_vaild_page_block_in_chip(int *plane_to_clean,ssd_element_metadata *metadata,ssd_t *s)
{
	int min_valid[s->params.num_parunits];
	int i = 0,chip_num;
	for(i = 0;i < s->params.num_parunits;i++)
	{
		min_valid[i] = s->params.pages_per_block - 1;
		plane_to_clean[i] = -1;
	}
	for(i = 0;i < s->params.blocks_per_element;i++)
	{
		if(!ssd_can_clean_block(s, metadata, i))continue;
		chip_num = (metadata->block_usage[i].plane_num)/SSD_PLANES_PER_PARUNIT(s);
		assert(chip_num < s->params.num_parunits);
		if(min_valid[chip_num] > metadata->block_usage[i].num_valid)
		{
			min_valid[chip_num] = metadata->block_usage[i].num_valid;
			plane_to_clean[chip_num] = metadata->block_usage[i].plane_num;
		}
	}
	for(i = 0;i < s->params.num_parunits;i++)
	assert(plane_to_clean[i] < SSD_PLANES_PER_PARUNIT(s)*(i+1));
	//printf("ytc94u %d %d\n",plane_to_clean[0],plane_to_clean[1]);
}
void find_min_vaild_page_plane(int *plane_to_clean,ssd_element_metadata *metadata,ssd_t *s)
{
	int i = 0,min_free;
	min_free = metadata->plane_meta[0].free_blocks;
	plane_to_clean[0] = 0;
	for(i = 1;i < 4;i++)
	{
		if(metadata->plane_meta[i].free_blocks < min_free)
		{
			min_free = metadata->plane_meta[i].free_blocks;
			plane_to_clean[0] = i;
		}
	}
	min_free = metadata->plane_meta[4].free_blocks;
	plane_to_clean[1] = 4;
	for(i = 5;i < 8;i++)
	{
		if(metadata->plane_meta[i].free_blocks < min_free)
...
double ssd_clean_element_copyback(int elem_num, ssd_t *s)
{
    ...
    for (i = 0; i < SSD_PARUNITS_PER_ELEM(s); i ++) {
        if ((plane_to_clean[i] = ssd_start_cleaning_parunit(i, elem_num, s)) != -1) {
	   				// find_min_vaild_page_plane(plane_to_clean,metadata,s);
            find_min_vaild_page_block_in_chip(plane_to_clean,metadata,s);
				    clean_req = 1;
	  			  GC_count_per_plane += 2;
		break;
        }
    }
    if(clean_req == 1)
...
double ssd_clean_element(ssd_t *s, int elem_num)
{
    ...
    if (s->params.copy_back == SSD_COPY_BACK_DISABLE) 
		...
		{	
			...
			if(pf_gc_data == NULL)
			{
				pf_para = fopen("para_file","r");
				assert(pf_para);
				fscanf(pf_para,"%s %s %s\n",tmp1,tmp2,tmp3);
				fclose(pf_para);
			
				pf_para =  fopen("para_file2","r");
				assert(pf_para);
				fscanf(pf_para,"%s\n",tmp4);
				fclose(pf_para);

				sprintf(gc_data_file,"gc_%s_%s.txt",tmp2,tmp4);
				pf_gc_data = fopen(gc_data_file,"w");
				assert(pf_gc_data);
			}
#endif
...
{
			// die_unit*planes_per_die
    return (parunit_num*SSD_PLANES_PER_PARUNIT(s) + (plane_num+1)%SSD_PLANES_PER_PARUNIT(s));
}
/*int ssd_start_cleaning_parunit_(int parunit_num, int elem_num, ssd_t *s)
{
    int i;
    int start;
