 ...
 * othewise we use the allocation method according to the locality
 * */
int striping_allocation;
...
void ssd_element_metadata_init(int elem_number, ssd_element_metadata *metadata, ssd_t *currdisk)
{
    ...
    unsigned int bsn = 1;
		unsigned int locality_logical_page_num;
    ...
    usable_blocks = usable_blocks_per_plane * currdisk->params.planes_per_pkg;
    //////////////////////////////////////////////////////////////////////////////
    //when we inited the first elem ,we allocation the mapping table  ytc94u
    if(elem_number == 0)
    {
    	lba_table = malloc(sizeof(page_level_mapping)*usable_blocks*SSD_DATA_PAGES_PER_BLOCK(currdisk)*currdisk->params.nelements);
			table_size = usable_blocks*SSD_DATA_PAGES_PER_BLOCK(currdisk)*currdisk->params.nelements;
			assert(lba_table != NULL);
    }
    //////////////////////////////////////////////////////////////////////////////
    ...
    for (i = 0; i < (unsigned int)currdisk->params.planes_per_pkg; i ++) {
        int w_blocks_to_skip,r_blocks_to_skip,g_blocks_to_skip;
        ...
        switch(plane_block_mapping) {
            ...
            case PLANE_BLOCKS_FULL_STRIPE:
                w_blocks_to_skip = (currdisk->params.planes_per_pkg * usable_blocks_per_plane) + i;
								r_blocks_to_skip = (currdisk->params.planes_per_pkg * (usable_blocks_per_plane + 1)) + i;
								g_blocks_to_skip = (currdisk->params.planes_per_pkg * (usable_blocks_per_plane + 2)) + i;
                break;
        ...
        metadata->plane_meta[i].w_active_page = w_blocks_to_skip*currdisk->params.pages_per_block;
				metadata->plane_meta[i].r_active_page = r_blocks_to_skip*currdisk->params.pages_per_block;
				metadata->plane_meta[i].g_active_page = g_blocks_to_skip*currdisk->params.pages_per_block;
    ...
    switch(plane_block_mapping) {
        ...
        case PLANE_BLOCKS_FULL_STRIPE: //the program will run there
    ...
    ASSERT(metadata->active_page == metadata->plane_meta[0].w_active_page);
   ...
   // }
    ...
    ppage = 0;
    i = 0;
		locality_logical_page_num = elem_number*SSD_DATA_PAGES_PER_BLOCK(currdisk);
    while (i < export_size) 
		{
        int pgnum_in_gang;
        int pp_index;
        ...
        if (ssd_last_page_in_block(ppage, currdisk)) {
            ...
            metadata->block_usage[block].state = SSD_BLOCK_SEALED;
						if(striping_allocation == 1)
	    				metadata->block_usage[block].rw_intensive = 1;//read intensive
						else
							metadata->block_usage[block].rw_intensive = 2;
            // go to next block
            ppage ++;
        ...
        // populate the lba table
				...
				if(striping_allocation == 1)
				{
					lba_table[i*currdisk->params.nelements+elem_number].elem_number = elem_number;
					lba_table[i*currdisk->params.nelements+elem_number].ppn = ppage;
				}
				else
				{
					lba_table[locality_logical_page_num].elem_number = elem_number;
					lba_table[locality_logical_page_num].ppn = ppage;
				}
        ...
        if(striping_allocation == 1)
				{
					metadata->block_usage[block].page[pp_index] = i*currdisk->params.nelements+elem_number;
				}
				else
				{
					metadata->block_usage[block].page[pp_index] = locality_logical_page_num;
				}
        ...
        i ++;
				...
				//go to the next logical page 
				locality_logical_page_num++;
				//after we fill a block ,the logical page will skip to the next logical block in the same package
				if(locality_logical_page_num % SSD_DATA_PAGES_PER_BLOCK(currdisk) == 0)
				{
					locality_logical_page_num += (currdisk->params.nelements-1)*SSD_DATA_PAGES_PER_BLOCK(currdisk);
				}
    ...
    switch(currdisk->params.copy_back) {
        ...
        case SSD_COPY_BACK_ENABLE:
            for (i = 0; i < (unsigned int)currdisk->params.planes_per_pkg; i ++) 
	    			{			
								//w active page
								int plane_active_block = SSD_PAGE_TO_BLOCK(metadata->plane_meta[i].w_active_page, currdisk);
                ...
                metadata->block_usage[plane_active_block].bsn = bsn ++;
								metadata->block_usage[plane_active_block].rw_intensive = 2;
                metadata->tot_free_blocks --;
                metadata->plane_meta[i].free_blocks --;
		
								//r active page
								plane_active_block = SSD_PAGE_TO_BLOCK(metadata->plane_meta[i].r_active_page, currdisk);

                bitpos = ssd_block_to_bitpos(currdisk, plane_active_block);
                ssd_set_bit(metadata->free_blocks, bitpos);
                metadata->block_usage[plane_active_block].state = SSD_BLOCK_INUSE;
                metadata->block_usage[plane_active_block].bsn = bsn ++;
                metadata->block_usage[plane_active_block].rw_intensive = 1;
								metadata->tot_free_blocks --;
                metadata->plane_meta[i].free_blocks --;
		
								//g active page
								plane_active_block = SSD_PAGE_TO_BLOCK(metadata->plane_meta[i].g_active_page, currdisk);
...
void ssd_initialize (void)
{
  ...
  int i, j;
	FILE *para_pf;
	/*
	 * get para from para_file2
	 * */
	para_pf = fopen("para_file2","r");
	assert(para_pf);
	fscanf(para_pf,"%u",&striping_allocation);
	fclose(para_pf);
...
*/
   ...
   for (i=0; i<numssds; i++) {
      ...
      //currdisk->params.alloc_pool_logic = SSD_ALLOC_POOL_GANG;
