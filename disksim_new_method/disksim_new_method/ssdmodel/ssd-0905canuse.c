...
#define RWRATIO 0.4
#define EVICTWINDOW 0.5


int h_data[TOTAL_NODE][LRUSIZE]={0};

int LPN_RWtimes[TOTAL_NODE][2]={0};//0:read count/1:write count
int State0=0, State1=0, State2=0;//ch
int kick_block_strip_node=0,kick_block_strip_sumpage=0;
int have_hinted_node=0;


int kick_node=0,kick_sum_page=0,kick_LG_block_striping=0;
...
typedef struct _current_block_info //��structure���O���۸�cur blk�Y�쪺�O���@��LB(���]�O�Y��W-intensive��page)���qoffset�}�l�s�� cur_mark_cnt��pages
{
...
struct ssd_information
{
  unsigned int node_page_nm[TOTAL_NODE][LRUSIZE];
};
/* 
...
//void check_for_striping_node(buffer_cache *ptr_buffer_cache);
...
struct Lg_access_count Lg_rw_count[TOTAL_NODE];
...
//FILE *outfile = fopen("/home/osnet/disksim_blas_lpb/src/outfile.txt","r");
FILE *cc,*dd ;
lru_node glob_bc;
struct ssd_information myssd;
//-------------------------------------------------------------------------------------
#ifndef sprintf_s
...
void find_the_locality_page(int *striping_node,unsigned int *lpn,unsigned int *seq_size)
{
	...
	*seq_size = 1;
	...
	if(((*lpn)%LRUSIZE) + *seq_size > LRUSIZE)
		rw_count[*striping_node].rw_replacement = STRIPING; // sh--��LB�Qscattered�������L��flag�� STRIPING
        ...
        if (read_total > 0) {
            ...
            for (i = 0; i < read_total; i ++) {
              ...
              if (schtime < read_reqs[i]->schtime) {// find the maximum time taken by a request
                  schtime = read_reqs[i]->schtime;  //sh-- �̫�schtime�N�Olargest execution time of read req.
              }
            ...
            for (i = 0; i < write_total; i ++) {
              ...
              write_reqs[i]->org_req->time = simtime + schtime + write_reqs[i]->schtime;
							 ...
							 * when the tempptr2 == NULL,it represent the request is the striping write in the background
							 sh--there are two kinds of write: 1. user request, 2. scatter request(��tempptr2�Ӥ���)
							 * */
...
static void ssd_media_access_request_element (ioreq_event *curr)
{
  ...
  //printf("\nreq_check=%d |blkno=%d |bcount=%d\n",req_check, blkno, count);
  //fprintf(lpb_ppn, "\nreq_check=%d |blkno=%d |bcount=%d\n",req_check, blkno, count);
    ...
    //record read times 
	 ...
	 /*
	   * the follow code is used for read request...
          */
    //fprintf(lpb_ppn, "read req\t");
    //int lnn,node_count;
    assert(curr->flags&READ);
    //printf("assert(curr->flags&READ);\n");
    /* if(curr->bcount >= LARGE_REQ)
    {
      lnn=(curr->blkno/8)/LRUSIZE;//=21
      //fprintf(lpb_ppn, "WIC--|Lg_rw_count[%d].WIC=%d\n", lnn, Lg_rw_count[lnn].WIC);
      if(Lg_rw_count[lnn].WIC>0)
        Lg_rw_count[lnn].WIC--;
      node_count=node_count-((lnn+1)*8*LRUSIZE-curr->blkno);
      while(node_count>0)
      {
        lnn=lnn+1;
        if(Lg_rw_count[lnn].WIC>0)
          Lg_rw_count[lnn].WIC--;
        node_count=node_count-(8*LRUSIZE);
      }
    }*/
...
void total_serve_time(ioreq_event *parent)
{
    ...
    //printf("read_total_serve_time=%lf\n", read_total_serve_time);
	...
	{
		write_request_count++;
		write_total_serve_time += simtime - Y_arr_time;
    //assert(write_total_serve_time >= 0);
    //assert(write_total_serve_time < 10000000000);
    //printf("simtime=%lf,parent->arrive_time=%lf,write_total_serve_time=%lf\n", simtime, Y_arr_time, write_total_serve_time);
	}
}
/*
 *
...
void ssd_complete_parent(ioreq_event *curr, ssd_t *currdisk)
{
    ...
    //printf("curr->tempptr2 == NULL\n");
		striping_request_finished();
		return ;
	}
	parent = curr->tempptr2;
  //printf("parent->arrive_time =%lf\n", parent->arrive_time);
  //printf("parent->blkno=%d\n", parent->blkno);
  //printf("parent->plane_num=%d\n", parent->plane_num);
  //assert(parent->plane_num >= 0 && parent->plane_num <= 7);
...
static void ssd_access_complete_element(ioreq_event *curr)
{
   ...
   //printf("ssd_access simtime=%lf\n", simtime);
   //printf("ssd_access_complete_element//curr->page no=%d //elem_num=%d\n", curr->blkno/8, elem_num);
...
void init_buffer_cache(buffer_cache *ptr_buffer_cache)
{
 cc = fopen("check.txt","w+");
 dd = fopen("check2.txt","w+");
	FILE *pf2,*pf;
...
int check_which_node_to_evict(buffer_cache *ptr_buffer_cache)
{
  ...
  for(i=0;i<EW;i++)//from lru find 64 node
  {
    ...
    {
      node_num=i;
      fprintf(outputssd, "2967 if(c_node == ptr_buffer_cache->ptr_head)|node_num=%d\n",node_num);
...
void add_and_remove_page_to_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache)
{
  ...
  unsigned int logical_add,myoffset_in_node;
  static unsigned int channel_num = 0,plane = 0;
	while(count > 0)
	{ 
    //printf( "\ncount=%d\tblkno=%d\t",count,blkno);
    //fprintf(lpb_ppn, "\ncount=%d\tblkno=%d\t",count,blkno);
    ...
    //lru=my_buffer_cache2->hash[logical_node_num % HASHSIZE];
   
    lru->logical_node_num=lpn/LRUSIZE;
    //glob_bc.logical_node_num=lpn/LRUSIZE;
    int i;
    for(i=0;i<req_RW_count->page_count;i++)
    {
      if(req_RW_count->page[i].page_num == lpn)
  ...
  //   //lru = current_block[channel_num][plane].ptr_lru_node;
  //   //myoffset_in_node = current_block2[channel_num][plane].offset_in_node;
  //   //glob_bc->logical_node_num=lpn/LRUSIZE;
  //   //lru->page[offset_in_node].his_strip=glob_bc->page[offset_in_node].his_strip;

  //   //fprintf(dd,"%d\n",lpn);
  //   //fprintf(dd,"lpn/LRUSIZE:%d logical_node_num:%d\n",logical_node_num,glob_bc.logical_node_num);
  //   //fprintf(dd,"logical_node_num:%d\n",glob_bc.logical_node_num);
    
       
  // //if(myssd.node_page_nm[logical_node_num][offset_in_node]==1)
  // //{
  //   //if(glob_bc.page[offset_in_node].strip==1)
  // /*   if(curr->bcount < LARGE_REQ)
  //   {
  //       flag=0;
  //       flag=Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
  //       fprintf(dd,"%d[%d]:%d Physical\n",logical_node_num,offset_in_node,myssd.node_page_nm[logical_node_num][offset_in_node]);
  //   }*/
  //   if(myssd.node_page_nm[logical_node_num][offset_in_node]==1 && LPN_RWtimes[logical_node_num][1]>LPN_RWtimes[logical_node_num][0])
  //   {
  //       flag=0;
  //       flag=Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
  //       //fprintf(dd,"%d[%d]:%d Physical\n",logical_node_num,offset_in_node,myssd.node_page_nm[logical_node_num][offset_in_node]);
  ...
  //   }
  //   //else if(glob_bc.page[offset_in_node].strip==2)
  //   else if(myssd.node_page_nm[logical_node_num][offset_in_node]==2)
  //   {
  //       flag=1;
  //       LARGEcount++;
  //       Y_add_Lg_page_to_cache_buffer(lpn,ptr_buffer_cache);
  //       //fprintf(dd,"%d[%d]:%d logical\n",logical_node_num,offset_in_node,myssd.node_page_nm[logical_node_num][offset_in_node]);
  //       //fprintf(dd,"%d\n",lpn);

  //   }
  //   else if(myssd.node_page_nm[logical_node_num][offset_in_node]==0)
  //   {
  //   /* if(curr->bcount >= LARGE_REQ)   ///original
  //    {
  //      flag=1;
  //      LARGEcount++;
  //      Y_add_Lg_page_to_cache_buffer(lpn,ptr_buffer_cache);
  ...
  //     LARGEcount++;
  //     Y_add_Lg_page_to_cache_buffer(lpn,ptr_buffer_cache);*/
  //     //fprintf(dd,"%d[%d] read:%d write:%d logical\n",logical_node_num,offset_in_node,LPN_RWtimes[logical_node_num][0],LPN_RWtimes[logical_node_num][1]);
  //     /* if(LPN_RWtimes[logical_node_num][0]==0 && LPN_RWtimes[logical_node_num][1]!=0) 
  //     {
  //        flag=0;
  //        flag=Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
  //     }
  ...
  //   }
    ...
    //Cp_node=glob_bc->ptr_current_mark_node;
    ...
    //Pg_node = glob_bc->hash_Pg[physical_node_num % HASHSIZE];
    //Pg_node = ptr_buffer_cache->hash_Pg[physical_node_num % HASHSIZE];
    //fprintf(lpb_lpn, "%d\n", lpn);
    ...
    //printf("elen_num1=%d,ppn1=%d,lpn=%d\n", elem_num1, ppn1,lpn);
    //printf("lpn = %d\n", lpn);
    //fprintf(lpb_ppn, "lpn = %d\n", lpn);
    //fprintf(lpb_ppn, "physical_node_num = %d\n", physical_node_num);
    //fprintf(lpb_ppn, "phy_node_offset = %d\n", phy_node_offset);
    //ptr_buffer_cache->ptr_current_mark_node->StripWay=1;
  //if(find_page_in_cache_buffer(lpn,&my_buffer_cache)==0)
  //{
  ...
  else
  {
    ...
    for(i=0;i<LRUSIZE;i++)
    {
      if(Pg_node->page[i].exist == 1)
      {
        ch=Pg_node->page[i].channel_num;
        p=Pg_node->page[i].plane;
    ...
    if(ch>=0&&p>=0)
    {
      ...
      //if(h_data[physical_node_num][phy_node_offset]==1)
      if(h_data[logical_node_num][offset_in_node]==1)
      //if(h_data[Pg_node->logical_node_num][i]==1)  
      //if(switchflag==1)
      {
         flag=0;
         flag=Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
         //fprintf(cc, "\nPg_node->page[phy_node_offset].strip:%d\tphysical\t",switchflag);
         fprintf(cc, "\n%d[%d]\tphysical\t",logical_node_num, offset_in_node);
         //fprintf(cc, "\n%d[%d]\tphysical\t",physical_node_num,phy_node_offset);
      }
      else
      {
        flag=1;
        LARGEcount++;
        Y_add_Lg_page_to_cache_buffer(lpn,ptr_buffer_cache);  
        //fprintf(cc, "\nPg_node->page[phy_node_offset].strip:%d\tlogical\t",switchflag);
        //add_page_to_cache_buffer(lpn,ptr_buffer_cache);
        fprintf(cc, "\n%d[%d]\tlogical\t",logical_node_num, offset_in_node);
        //fprintf(cc, "\n%d[%d]\tlogical\t",physical_node_num,phy_node_offset);
      }
     }
  }*/
  //}

     /* if(check_for_striping_node(ptr_buffer_cache)==1)
      {
         flag=0;
         flag=Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
         //fprintf(cc, "\nPg_node->page[phy_node_offset].strip:%d\tphysical\t",switchflag);
      }
      else if(check_for_striping_node(ptr_buffer_cache)==2)
      {
        flag=1;
        LARGEcount++;
        Y_add_Lg_page_to_cache_buffer(lpn,ptr_buffer_cache);  
        //fprintf(cc, "\nPg_nodde->page[phy_node_offset].strip:%d\tlogical\t",switchflag);
        //add_page_to_cache_buffer(lpn,ptr_buffer_cache);
      }
      else
      {
    ...
    */
    ...
    /* if(ptr_lru_node->page[offset_in_node].strip==2)
    {
      ...
      LARGEcount++;
      Y_add_Lg_page_to_cache_buffer(lpn,ptr_buffer_cache);
      fprintf(cc, "\npage:%d\tlogical\t",ptr_lru_node->page[offset_in_node]);
    }
    else if(ptr_lru_node->page[offset_in_node].strip==1)
    {
      flag=0; 
      Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
      fprintf(cc, "\npage:%d\tPhysical\t",ptr_lru_node->page[offset_in_node]);
      //flag=Y_add_Pg_page_to_cache_buffer(lpn,ptr_buffer_cache);
    }
     else
    { 
    ...
    {
      //fprintf(cc, "\npage:%d\tlogical\t",mylpn);
    }*/
    //mystrip=check_which_node_to_evict(ptr_buffer_cache);
    ...
    /*  while(1)
      {
        ...
        for(i=0;i<LRUSIZE;i++)
        {
          if(lru->page[i].exist != 0)
            //printf("[%d]%d,", i, lru->page[i].lpn);
        }
        //printf("\n");
	...
	if(block_level_lru_no_parallel == 0)//���Ұ�proposed PaW
	{
...
int Y_add_Pg_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
  ...
  if(Pg_node == NULL)
  {
    ...
    //fprintf(lpb_ppn, "if(Pg_node == NULL)\tphysical_node_num=%d\n", physical_node_num);
      ...
      add_a_node_to_buffer_cache(lpn,physical_node_num,phy_node_offset,ptr_buffer_cache,flag=0);
...
int find_idle_channel(unsigned int *channel_num,unsigned int *plane)
{
  ...
  for(i = 0;i < CHANNEL_NUM;i++)
  {
    ...
    {   //current_mark_count: ���Ocurrent �Y�쪺 WI pages count �C �Ycurrent blk�Y�쪺�OR-intensive pages�A�N�ݭnlink list(ptr_read_intensive_buffer_page)�ӰO���o��pages(�]���L�̨Ӧۤ��P��LB)
      if(current_block[i][j].current_mark_count == 0 && current_block[i][j].ptr_read_intensive_buffer_page == NULL) //sh -- ��current ���򳣨S�Y��~
      {
        *channel_num = i;
        *plane = j;
        return 0;
...
int check_for_striping_node(buffer_cache *ptr_buffer_cache)
{
  //ptr_buffer_cache=glob_bc;
  ...
  for(i = 0;i < LRUSIZE;i++)
  {
    if(ptr_lru_node->page[i].exist == 2)
    {
      if(ptr_lru_node->page[i].strip==1)
      {
        return 1;
      }
      else if(ptr_lru_node->page[i].strip==2)
      {
        return 2;
      }
      else
...
/*  if(ptr_buffer_cache->ptr_current_mark_node == ptr_buffer_cache->ptr_head)
  ...
  for(i = 0;i < LRUSIZE;i++)
  {
    if(ptr_lru_node->page[i].exist == 1)
    {
      ...
      if(have_channel == -1)
      {
        //fprintf(lpb_ppn, "|||||||can't find_idle_channel\n");
        return -2;
      }
      ...
      ptr_buffer_cache->current_mark_offset =i;
      ptr_lru_node->page[i].strip=2;//page striping
      //glob_bc.page[i].strip=2;
      ps++;
      //h_data[ptr_lru_node->logical_node_num][i]=1; 
      //fprintf(lpb_ppn, "***%d[%d].channel=%d,plane=%d\n",ptr_buffer_cache->ptr_current_mark_node->logical_node_num,i,channel_num,plane);
      //fprintf(cc, "\nPg_nodde->page[phy_node_offset].strip:%d\tlogical\t",switchflag);
      //fprintf(cc, "***%d[%d].channel=%d,plane=%d page striping\n",ptr_buffer_cache->ptr_current_mark_node->logical_node_num,i,channel_num,plane);
...
void remove_mark_in_the_node(lru_node *ptr_lru_node,buffer_cache *ptr_buffer_cache)
{
  //fprintf(lpb_ppn,"%d:ptr_lru_node->rw_intensive=%d\n", ptr_lru_node->logical_node_num, ptr_lru_node->rw_intensive);
	unsigned int i = 0;
  ptr_lru_node->rw_intensive = 2;
    ...
    //  plane = min_valid_page_in_plane(sta_die_num,currdisk,channel_num);
      ...
      else if(ptr_lru_node->page[offset_in_node].exist == 2 && ptr_lru_node->page[offset_in_node].channel_num == channel_num && ptr_lru_node->page[offset_in_node].plane==plane)
      {
       ...
       //printf("* if the page already been marked|\n");
        ...
        k++;
        if(ptr_lru_node->StripWay == 0 && ptr_lru_node->group_type==0)
        {
          kick_block_striping_page_count++;
          ptr_lru_node->page[offset_in_node].strip = 0;
          ptr_lru_node->page[offset_in_node].his_strip =2 ;
          glob_bc.page[offset_in_node].his_strip=2;
          glob_bc.page[offset_in_node].strip=2;
          glob_bc.logical_node_num=ptr_lru_node->logical_node_num;
          /* node_count++;
          ...
          fprintf(cc,"%d[%d]:%d Bs\n",ptr_lru_node->logical_node_num,offset_in_node,ptr_lru_node->page[offset_in_node].his_strip);
        }
        else if(ptr_lru_node->StripWay == 1 && ptr_lru_node->group_type==1)
        {
          kick_page_striping_page_count++;
          ptr_lru_node->page[offset_in_node].strip = 0;
          ptr_lru_node->page[offset_in_node].his_strip =1 ;
          glob_bc.page[offset_in_node].his_strip=1;
          glob_bc.page[offset_in_node].strip=1;
          glob_bc.logical_node_num=ptr_lru_node->logical_node_num;
...
void add_to_ioqueue(ioreq_event* curr,unsigned int channel_num,unsigned int plane,unsigned int lpn,unsigned int rw_intensive)
{
  ...
  channel_plane_write_count[channel_num][plane]++;
	...
	ssd_t *currdisk;	
	ioreq_event *child = (ioreq_event *)getfromextraq();
  ioreq_event *parent = (ioreq_event *)getfromextraq();
	currdisk = getssd (curr->devno);
  ...
  //printf("child->tempptr2->arrive_time=%lf\n", parent->arrive_time);
...
void show_result(buffer_cache *ptr_buffer_cache)
{
	...
	statistic_the_data_in_every_stage();
  printf("[Yifen] LARGEcount = %d , Pg_hit_Lg=%d\n",LARGEcount, Pg_hit_Lg);
  printf(GREEN"[LPB] kick_page_striping_page_count=%d, kick_block_striping_page_count=%d\n"NONE, kick_page_striping_page_count, kick_block_striping_page_count);
  ...
  printf(" ytc94u total_live_page_cp_count3 = %d\n",total_live_page_cp_count2 );
	if(total_gc_count != 0)   	
	printf("ytc94u average live page num in  victim block = %lf\n",(double)total_live_page_cp_count/(double)total_gc_count);
  ...
  printf("block striping mark:%d\n",bs);
  printf("page striping mark:%d\n",ps);
