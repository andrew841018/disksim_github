...
current_block_info current_block[CHANNEL_NUM][PLANE_NUM],current_block_1[30][30];
...
int Y_add_Pg_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
  ...
  int i,j;
  if(init==1){
    FILE *oracle=fopen("request.txt","r");
    char buf[1024];
    char *substr=NULL;
    const char *const delim=" ";
    int block_number,curr_time;
  ...
  if(Pg_node == NULL)
  {
    ...
    for(i=0;i<CHANNEL_NUM;i++){
      for(j=0;j<PLANE_NUM;j++){
        if(current_block[i][j].ptr_lru_node==NULL || current_block[i][j].current_mark_count==0){
    ...
    //current block have space,assign corresponding current_mark_count to it(should equal to page count)
    for(i=0;i<LRUSIZE;i++){
	  if(assign==0){
		break;
	  }
    ...
    if(assign==0){
		for(i=0;i<30;i++){
			for(j=0;j<30;j++){
				if(current_block_1[i][j].ptr_lru_node==NULL){
					current_block_1[i][j].ptr_lru_node=ptr_buffer_cache->hash_Pg[physical_node_num % HASHSIZE];					
					assign=1;
					break;
				}
			}
  ...
  else
  {
    ...
    //means block is in the current_block_1
    if(assign==0){
		...
		for(i=0;i<30;i++){
			for(j=0;j<30;j++){
				if(current_block_1[i][j].ptr_lru_node==Pg_node){
					...
					curr1_channel=i;
					curr1_plane=j;
					printf("block hit,but in the current_block_1\n");
					break;			
				}
			}
			if(assign==1){
		...
		assert(assign==1);
		assign=0;
		//make sure curr block is in current_block_1,check whether current_block have free space 
		for(i=0;i<CHANNEL_NUM;i++){
			for(j=0;j<PLANE_NUM;j++){
				if(current_block[i][j].ptr_lru_node==NULL){
					...
					current_block_1[curr1_channel][curr1_plane].ptr_lru_node=NULL;
					assign=1;
					break;
				}
			}
	...
	//
    for(i=0;i<LRUSIZE;i++){
	//curr block is in current_block_1 but current_block doesn't have free space
	  if(assign==0){
		break;
	  }
      if(current_block[channel][plane].ptr_lru_node->page[i].exist!=0){
...
void remove_a_page_in_the_node(unsigned int offset_in_node,lru_node *ptr_lru_node,buffer_cache *ptr_buffer_cache,unsigned int verify_channel,unsigned int verify_plane,int flag)
{
  ...
  //printf("channel_num=%d |verify_channel=%d\n", channel_num,verify_channel);
	...
	ptr_buffer_cache->total_buffer_page_num --;
	current_block[channel_num][plane].current_mark_count --;
	assert(current_block[channel_num][plane].current_mark_count==ptr_lru_node->buffer_page_num);
	current_block[channel_num][plane].current_write_offset ++;
	if(ptr_lru_node->buffer_page_num == 0)
	{
...
// }
int remove_channel=-1,remove_plane=-1;
void remove_from_hash_and_lru(buffer_cache *ptr_buffer_cache,lru_node *ptr_lru_node,int flag)
{
	unsigned int logical_node_num = ptr_lru_node->logical_node_num;
	//remove node from hash 
  ...
  else if(flag==1)
  {
	...
	free(ptr_lru_node);
  current_block[remove_channel][remove_plane].ptr_lru_node=NULL;
  ptr_buffer_cache->total_buffer_block_num--;
}
...
void kick_page_from_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache,int flag)
{
  ...
  static unsigned int channel_num = 0,plane = 0,sta_die_num = 0,i = 0,j;
  unsigned int offset_in_node,logical_add;
  lru_node *ptr_lru_node;
  ssd_t *currdisk = getssd(curr->devno);
  curr->tempint2 = 0;
  ...
  while(ptr_buffer_cache->total_buffer_page_num > ptr_buffer_cache->max_buffer_page_num)
  {
    int k=0,channel,plane;
    double max=-1; 
    kick=1;
    //找出write buffer內最久才要被存取的block
    for(i=0;i<CHANNEL_NUM;i++){
      for(j=0;j<PLANE_NUM;j++){
        if(current_block[i][j].ptr_lru_node!=NULL){
          if(max<current_block[i][j].ptr_lru_node->next_acc_time){
            max=current_block[i][j].ptr_lru_node->next_acc_time;
            channel=i;
            plane=j;
          }
