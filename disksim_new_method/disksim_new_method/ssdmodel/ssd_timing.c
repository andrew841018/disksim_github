...
#define ASSOCIATE_LOGICAL_BLOCK
extern page_level_mapping *lba_table;
extern int req_count;
extern int fill_block_count;
extern int total_logical_block_count;
 ...
 */
double _ssd_write_page_osr(ssd_t *s, ssd_element_metadata *metadata, int lpn,int elem_num,int rwg)
{
    double cost;
    ...
    int i , j , logical_block_count;
		ssd_element_metadata *prev_metadata;
   	// see if this logical page no is already mapped.
    //if (metadata->lba_table[lpn] != -1) {
		if (lba_table[lpn].ppn != -1) {
        ...
        unsigned int prev_page = lba_table[lpn].ppn;
				unsigned int prev_elem = lba_table[lpn].elem_number;
        ...
        unsigned int pagepos_in_prev_block = prev_page % s->params.pages_per_block;
				prev_metadata = &s->elements[prev_elem].metadata;
        unsigned int prev_plane = prev_metadata->block_usage[prev_block].plane_num;

        // make sure the version numbers are correct
        //ssd_assert_page_version(prev_page, active_page, metadata, s);

        if (prev_metadata->block_usage[prev_block].page[pagepos_in_prev_block] != lpn) {
    ...
    lba_table[lpn].ppn = active_page;
    lba_table[lpn].elem_number = elem_num;
    ...
    metadata->active_page = active_page + 1;
		...
		if(rwg == 0)
    		metadata->plane_meta[active_plane].w_active_page = metadata->active_page;
    else if(rwg == 1)
				metadata->plane_meta[active_plane].r_active_page = metadata->active_page;
    else if(rwg == 2)
				metadata->plane_meta[active_plane].g_active_page = metadata->active_page;
    ...
    if (ssd_last_page_in_block(metadata->active_page, s)) {
        ...
        //printf("SUMMARY: lpn %d active pg %d\n", lpn, active_page);
 				...
 				/*
				 * the below code calculate the physical block associate what many logical block...
				 * the method is that find the previous page whether exist same logical blcok or not
				 * */
#ifdef ASSOCIATE_LOGICAL_BLOCK
				logical_block_count = 63;
				for(i = 1;i < 63; i++)
				{
					for(j = i - 1;j >= 0; j--)
					{
						if(metadata->block_usage[active_block].page[i]/s->params.pages_per_block == \
								metadata->block_usage[active_block].page[j]/s->params.pages_per_block)
						{
							logical_block_count --;
							break;
						}
				...
				fill_block_count ++;
				total_logical_block_count += logical_block_count;
#endif
		}
 ...
 */
void _ssd_alloc_active_block(int plane_num, int elem_num, ssd_t *s,int rwg)
{
    ...
    if (active_block != -1) {
        ...
        if(rwg == 0)
				{
					metadata->block_usage[active_block].rw_intensive = 2;
        	pm->w_active_page = active_block * s->params.pages_per_block;
				}	
				else if(rwg == 1)
				{	
					metadata->block_usage[active_block].rw_intensive = 1;
					pm->r_active_page = active_block * s->params.pages_per_block;
				}
				else if(rwg == 2)
				{
					metadata->block_usage[active_block].rw_intensive = 2;
					pm->g_active_page = active_block * s->params.pages_per_block;
				}
				metadata->active_page = active_block * s->params.pages_per_block;
    ...
    for (i = 0; i < total; i ++) {
        ...
        if (!reqs[i]->is_read) {
            ...
            if (s->params.alloc_pool_logic == SSD_ALLOC_POOL_PLANE) {
                ...
                j = reqs[i]->org_req->plane_num;//metadata->plane_to_write;
            		assert(j < s->params.planes_per_pkg);
						 		// do {
                  //  int active_block;
                 //   int active_bsn;
            ...
            //        if ((active_bsn > prev_bsn) ||
              //          ((active_bsn == prev_bsn) && (pm->active_page > (unsigned int)prev_page))) {
                        ...
                        p = metadata->plane_meta[j].parunit_num;
												if(p >= s->params.num_parunits)
												{
													assert(p < s->params.num_parunits);
												}
            ...
            if (size > 0) {
                ...
                if (r->is_read) {
                    parunit_op_cost[i] = s->params.page_read_latency;
										...
										if(r->org_req->rw_intensive&READ)
												active_page = metadata->plane_meta[plane_num].r_active_page;
										else
												active_page = metadata->plane_meta[plane_num].w_active_page;
					
                    if(ssd_last_page_in_block(active_page, s)) 
										{
			 									_ssd_alloc_active_block(plane_num, elem_num, s,r->org_req->rw_intensive&READ);
                    }
                    // issue the write to the current active page.
                    // we need to transfer the data across the serial pins for write.
										if(r->org_req->rw_intensive&READ)
		    								metadata->active_page = metadata->plane_meta[plane_num].r_active_page;
										else
												metadata->active_page = metadata->plane_meta[plane_num].w_active_page;
					
                    	parunit_op_cost[i] = _ssd_write_page_osr(s, metadata, lpn,elem_num,r->org_req->rw_intensive&READ);
                }
...
static double ssd_write_one_active_page(int blkno, int count, int elem_num, ssd_t *s)
{
    ...
    if (ssd_last_page_in_block(metadata->active_page, s)) {
        ...
        if (!cleaning_invoked ||
            ...
            _ssd_alloc_active_block(-1, elem_num, s,2);
        }
    }


    // issue the write to the current active page
    cost += _ssd_write_page_osr(s, metadata, lpn,elem_num,0);
    cost += ssd_data_transfer_cost(s, count);
