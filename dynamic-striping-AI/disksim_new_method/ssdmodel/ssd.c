...
void AI_predict_victim(buffer_cache *ptr_buffer_cache){
	...
	/*if(min_acc>(float)history[i]->record_dur_prior*-history[i]->pass_req_count*history[i]->overwrite_num){
		...
		if(history[i]->overwrite_num==0){//not overwrite and in LRU
			if(min_acc>(float)history[i]->duration_priority*1/history[i]->block_size){
				min_acc=(float)history[i]->duration_priority*1/history[i]->block_size;
				min_history_index=i;
			}
		}
		else{//overwrite:find max size block and in LRU
			if(max<history[i]->block_size*history[i]->pass_req_count){				
				max=history[i]->block_size*history[i]->pass_req_count;
				max_history_index=i;
			}			
		}
	}
...
void mark_for_specific_current_block(buffer_cache *ptr_buffer_cache,unsigned int channel_num,unsigned int plane)
{
	...
	if(assign==1){
		...
		printf("block size:%d\n",ptr_buffer_cache->ptr_current_mark_node->block_size);
...
void kick_page_from_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache,int flag)
{
  ...
  while(ptr_buffer_cache->total_buffer_page_num > ptr_buffer_cache->max_buffer_page_num)
  {
    ...
    //fprintf(lpb_ppn, "inin channel=%d,plane=%d\n", channel_num,plane);     
      ...
      while(remove_one_block==0){
		  if(no_page_can_evict == 0)
		  {
			...
			for(i=0;i<LRUSIZE;i++){
				if(target->page[i].exist==2 && target->StripWay==1){
	...
	assert(ptr_lru_node->buffer_page_num==0);
  }
  kick_count+=kick;
  assert(kick_count<=LRUSIZE);
  my_kick_node+=kick;
