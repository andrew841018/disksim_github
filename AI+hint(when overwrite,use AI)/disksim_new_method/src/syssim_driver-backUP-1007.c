    ...
    if(rep==1)
    {
      ...
      for(i=0;i<5;i++)
      {
        if(current_Point->Dirty == 1 && current_Point->Hint_Dirty == 1)
        {
          ...
          //fprintf(myoutput, "WBH dirty incoming hint page = %d\n", current_Point->blockNumber/8 );
...
int main(int argc, char *argv[]) 
{
  ...
  myoutput = fopen("src/syssim_2020","w");
  myoutput2 = fopen("src/syssim_2020-TWO","w");
  myoutput3 = fopen("src/syssim_2020-Three","w");
