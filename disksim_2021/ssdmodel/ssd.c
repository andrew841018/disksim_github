...
void A_kick_page_from_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache,int flag,int lpn);
...
void add_and_remove_page_to_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache)
{
  ...
  int physical_block_number=(lba_table[lpn].ppn+(lba_table[lpn].elem_number*1048576))/LRUSIZE;
  //kick_page_from_buffer_cache(curr,ptr_buffer_cache,flag);
  A_kick_page_from_buffer_cache(curr,ptr_buffer_cache,flag,physical_block_number);
...
int Y_add_Pg_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
  ...
  if(Pg_node == NULL)//(A),(C) enter there
  {
      ...
      A_add_a_node_to_buffer_cache(lpn,physical_node_num,phy_node_offset,ptr_buffer_cache,flag);
...
void A_add_a_node_to_buffer_cache(unsigned int lpn,unsigned int logical_node_num,unsigned int offset_in_node,buffer_cache * ptr_buffer_cache,int flag)
{
  ...
  ptr_buffer_cache->total_buffer_block_num++;
  char *substr=NULL;
  char buffer[1024];//1024 mean nothing,just make sure string  long enough.
...
/*sh-- (1.if cache hit. 2.which to kick?  3.where to kick?)*/
void A_kick_page_from_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache,int flag,int physical block number)
{
