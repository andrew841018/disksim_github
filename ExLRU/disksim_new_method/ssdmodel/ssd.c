...
{ 
  ...
  int hit_count;
  double AFP;
  int first_req_num,curr_req_num;
...
void add_and_remove_page_to_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache)
{
 ...
 /* int physical_node_num,phy_node_offset;
  for(i=0;i<global_HQ_size;i++){
	...
	if(special_used[physical_node_num]!=NULL){
		if(special_used[physical_node_num]->page[phy_node_offset].exist==0){
...
void add_a_page_in_the_node(unsigned int lpn,unsigned int logical_node_num,unsigned int offset_in_node,lru_node *ptr_lru_node,buffer_cache *ptr_buffer_cache,int flag)
{
    ...
    //fprintf(lpb_lpn, "w_hit\n");
		ptr_lru_node->page[offset_in_node].hit_count++;	
		ptr_lru_node->page[offset_in_node].curr_req_num=ReqCount;
		ptr_buffer_cache->w_hit_count ++;
		ptr_lru_node->page[offset_in_node].AFP=(double)ptr_lru_node->page[offset_in_node].hit_count/(ptr_lru_node->page[offset_in_node].curr_req_num-ptr_lru_node->page[offset_in_node].first_req_num);
		int i;
		ptr_lru_node->AFB=0;
		for(i=0;i<LRUSIZE;i++){
...
void mark_for_specific_current_block(buffer_cache *ptr_buffer_cache,unsigned int channel_num,unsigned int plane)
{
  ...
  //wait lru node is read intensive , add read lru node in channel
    ...
    if(p!=NULL){
		assert(p==ptr_buffer_cache->ptr_current_mark_node);
	}
	//mark write intensive node
    ...
    if(outout==1)
    {
      ...
      break;
		...
		//when enough mark page for current block...edit by andrew
	//	ssd_t *currdisk = getssd(0);
      ...
      while(k<8 || kick_block==0){
		  ...
		  if(no_page_can_evict == 0)
		  {
			...
			for(i=0;i<LRUSIZE;i++){
				...
				else if(p->page[i].exist==1){
					assert(0);
				}
			}
			assert(current_block[channel_num][plane].current_mark_count>0); 
			assert(current_block[channel_num][plane].ptr_lru_node!=NULL);
			assert(current_block[channel_num][plane].ptr_lru_node->select_victim==1);		
		  ...
		  else if(ptr_lru_node->page[offset_in_node].exist == 1)
		  {
			...
			for(i=0;i<64;i++)
			{
			  ...
			  if(channel_num==7 && plane==0){
				printf("lru channel:%d channel:%d lru plane:%d plane:%d page:%d exist:%d\n",ptr_lru_node->page[i].channel_num,channel_num,ptr_lru_node->page[i].plane,plane,i,ptr_lru_node->page[i].exist);
			  }
			}
			if(find_page == 0)
			{
