...
int init=1;
void init_struct(buf *wb,int block_index,int type){
  wb->block[block_index]=malloc(sizeof(buf));
  if(type==0){//init block    
    wb->block[block_index]->block_count=0;
    wb->block[block_index]->block_num=-1;
  ...
  if(type==1){//init sector
    ...
    wb->block[block_index]->sector[wb->block[block_index]->sector_index]->sector_num=-1;
    wb->block[block_index]->sector[wb->block[block_index]->sector_index]->sector_count=0;
  }
  if(type==2)//init both
    wb->block[block_index]->block_count=0;
    wb->block[block_index]->block_num=-1;
