...
static void ssd_media_access_request_element (ioreq_event *curr)
{
  ...
  req_check++;
  //printf("req_check=%d|curr->blkno=%d\n", req_check, curr->blkno);
  ssd_t *currdisk = getssd(curr->devno);
 ...
 //printf("req_check=%d|blkno=%d\n",req_check,blkno);
  ...
  //printf("curr->arrive_time=%lf\n", curr->arrive_time);
   ...
   if(!(curr->flags&READ))
   {
     //fprintf(outputssd, "curr->blkno=%d\n", curr->blkno);
      //fprintf(outputssd, "write curr->r_count=%d | curr->w_count=%d\n", curr->r_count, curr->w_count);
...
//666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666
  ...
  else
  {
    printf("state error!!\n");
    printf("state = %d\n", state);
    ptr_buffer_cache->ptr_current_mark_node = ptr_buffer_cache->ptr_head;
    strip_way = 0;
  }
  //printf("endend\n");
  if(strip_way==0)
...
void add_and_remove_page_to_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache)
{
  ...
  if(block_level_lru_no_parallel == 0)//¦³±Ò°Êproposed PaW
  {
    ...
    {// "first" time cache was filled up.
      ...
      int strip_way=-1;
      //printf("if(full_cache == 0 && ptr_buffer_cache->max_buffer_page_num < ptr_buffer_cache->total_buffer_page_num)\n");
      strip_way=check_which_node_to_evict(ptr_buffer_cache);

      //fprintf(outputssd, "strip_way=%d\n", strip_way);
      //printf("strip_way=%d\n", strip_way);
      //fprintf(outputssd, "ptr_buffer_cache->ptr_current_mark_node=%d\n", ptr_buffer_cache->ptr_current_mark_node->logical_node_num);
      //printf("ck3377\n");
      //ptr_buffer_cache->ptr_current_mark_node = ptr_buffer_cache->ptr_head->prev;
      //printf("ck3379\n");
      //fprintf(outputssd, "LRU ptr_buffer_cache->ptr_current_mark_node=%d\n", ptr_buffer_cache->ptr_current_mark_node->logical_node_num);
      ptr_buffer_cache->current_mark_offset = 0;
      //printf("ck3382\n");
      mark_for_all_current_block(ptr_buffer_cache);
      //fprintf(outputssd, "1111after mark_for_all_current_block(ptr_buffer_cache);\n");
    ...
    {
      mark_for_all_current_block(ptr_buffer_cache);
      //fprintf(outputssd, "2222after mark_for_all_current_block(ptr_buffer_cache);\n");
      
    }
  }
  //printf("before kick_page_from_buffer_cache\n");
  //unsigned long diff;
  ...
  //fprintf(outputssd,"!!!!!!kick_page_from_buffer_cache TIME = %ld\n",diff);

  //printf("ck3392\n");
  //fprintf(outputssd, "=======after kick_page_from_buffer_cache\n");
}
void add_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
...
void add_a_page_in_the_node(unsigned int logical_node_num,unsigned int offset_in_node,lru_node *ptr_lru_node,buffer_cache *ptr_buffer_cache )
{
  ...
  { 
    ...
    //fprintf(outputssd,"2ptr_lru_node->page[%d].lpn = %d|", offset_in_node, ptr_lru_node->page[offset_in_node].lpn);
    //fprintf(outputssd,"2page_RW_count->page_num = %d\n", page_RW_count->page_num);
...
/*  if(ptr_buffer_cache->ptr_current_mark_node == ptr_buffer_cache->ptr_head)
  ...
  for(i = 0;i < LRUSIZE;i++)
  {
    if(ptr_lru_node->page[i].exist == 1)
    {
      ...
      fprintf(cc, "whicknode:%d page:%d  pageS\n",ptr_lru_node,i);
  ...
  for(i = 0;i < CHANNEL_NUM;i++)
  {
    ...
    {   //current_mark_count: ¨ä¹ê¬Ocurrent ¦Y¨ìªº WI pages count ¡C ­Ycurrent blk¦Y¨ìªº¬OR-intensive pages¡A´N»Ý­nlink list(ptr_read_intensive_buffer_page)¨Ó°O¿ý³o¨Çpages(¦]¬°¥L­Ì¨Ó¦Û¤£¦PªºLB)
      if(current_block[i][j].current_mark_count == 0 && current_block[i][j].ptr_read_intensive_buffer_page == NULL) //sh -- ¦¹current ¤°»ò³£¨S¦Y¨ì~
      {
        ...
        //gettimeofday(&end, NULL);
        //diff=1000000 * (end.tv_sec-start.tv_sec)+ end.tv_usec-start.tv_usec;
        //fprintf(outputssd,"!!!!!!all chennel mark_for_specific_current_block TIME = %ld\n",diff);
...
void mark_for_specific_current_block(buffer_cache *ptr_buffer_cache,unsigned int channel_num,unsigned int plane)
{
  ...
  while(1)
  {
    ...
    if(ptr_buffer_cache->ptr_current_mark_node->page[ptr_buffer_cache->current_mark_offset].exist == 1 && ptr_buffer_cache->ptr_current_mark_node->StripWay==0)
    {
      ...
      fprintf(cc, "whicknode:%d page:%d  blockS\n",ptr_buffer_cache->ptr_current_mark_node ,ptr_buffer_cache->current_mark_offset);
      current_block[channel_num][plane].current_mark_count ++;
    }
    ...
    { 
      //fprintf(outputssd, "if(ptr_buffer_cache->current_mark_offset == LRUSIZE && current_block[%d][%d].current_mark_count > 0)\n", channel_num, plane);
      assert(ptr_buffer_cache->ptr_current_mark_node != ptr_buffer_cache->ptr_head);
      int strip_way = check_which_node_to_evict(ptr_buffer_cache);
      ...
      //fprintf(outputssd, "strip_way =%d\n",strip_way); 
      //fprintf(outputssd, "ptr_buffer_cache->ptr_current_mark_node=%d\n",ptr_buffer_cache->ptr_current_mark_node->logical_node_num); 
      //fprintf(outputssd, "3755 current_block[%d][%d].ptr_lru_node = %d|current_mark_count = %d\n", channel_num, plane, current_block[channel_num][plane].ptr_lru_node->logical_node_num, current_block[channel_num][plane].current_mark_count);
      //fprintf(outputssd, "3756 ptr_buffer_cache->current_mark_offset = %d\n", ptr_buffer_cache->current_mark_offset);
      ptr_buffer_cache->current_mark_offset = 0;
      while(strip_way == 1)
      {
        fprintf(outputssd,"3792 while(strip_way == 1)\n");
    ...
    {
      //fprintf(outputssd, "else if(ptr_buffer_cache->current_mark_offset == LRUSIZE)\n");
      //fprintf(outputssd, "3764 current_block[%d][%d].ptr_lru_node = %d|current_mark_count = %d\n", channel_num, plane, current_block[channel_num][plane].ptr_lru_node->logical_node_num, current_block[channel_num][plane].current_mark_count);
      ...
      current_block[channel_num][plane].offset_in_node = ptr_buffer_cache->current_mark_offset;
      //fprintf(outputssd, "check!!!!strip_way=%d\n",strip_way);
    }
    if(outout==1)
    {
      outout=0;
  ...
  assert(current_block[channel_num][plane].current_mark_count != 0);
  //fprintf(outputssd, "3892 current_block[%d][%d].ptr_lru_node = %d|current_mark_count = %d\n", channel_num, plane, current_block[channel_num][plane].ptr_lru_node->logical_node_num, current_block[channel_num][plane].current_mark_count);
  //fprintf(outputssd, "3893 .current_write_offset=%d \n", current_block[channel_num][plane].current_write_offset);
}
void remove_mark_in_the_node(lru_node *ptr_lru_node,buffer_cache *ptr_buffer_cache)
{
  unsigned int i = 0;
...
void show_result(buffer_cache *ptr_buffer_cache)
{
  ...
  printf(LIGHT_GREEN"[YIFEN] RWRATIO=%lf, EVICTWINDOW=%f\n"NONE, RWRATIO, EVICTWINDOW);
  printf(LIGHT_GREEN"[YIFEN] WB_size = %d\n"NONE, ptr_buffer_cache->max_buffer_page_num);
